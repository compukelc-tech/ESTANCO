...
function procesarReporteHistorico(filtro) {
  ...
  memoriaVentas.forEach(v => {
    if (!v.fecha) return; var f = new Date(v.fecha), d = f.getDate(), m = f.getMonth(), y = f.getFullYear(), apl = false;
    if (filtro === 'dia' && d === dH && m === mH && y === yH) apl = true;
    else if (filtro === 'mes' && m === mH && y === yH) apl = true;
    else if (filtro === 'ano' && y === yH) apl = true;
    else if (filtro === 'semana') { var ini = new Date(hoy); ini.setDate(ini.getDate() - ini.getDay()); ini.setHours(0,0,0,0); var fin = new Date(ini); fin.setDate(fin.getDate() + 6); fin.setHours(23,59,59,999); if (f >= ini && f <= fin) apl = true; }
    if (apl) { vB += v.total; cR += v.cost; gN += v.ganancia; if (v.tipoPago === 'Fiar' && v.estadoPago === 'Pendiente') fC += v.total; else rE += v.total; }
  });
  ...
  $('txtTotalVentasReporte').textContent = '$' + vB.toFixed(2); $('txtTotalReporte').textContent = '$' + rE.toFixed(2); $('txtFiadoReporte').textContent = '$' + fC.toFixed(2); $('txtCostoReporte').textContent = '$' + cR.toFixed(2); $('txtGananciaReporte').textContent = '$' + gN.toFixed(2);
